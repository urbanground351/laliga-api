...
def fetch_matches():
    # Kambi CDN - Tüm İspanya Futbol Ağı
    url = "https://eu-offering-api.kambicdn.com/offering/v2018/ub/listView/football/spain.json?lang=tr_TR&market=TR"
    
    final_data = {
        "last_updated": datetime.now().isoformat(),
        "source": "Kambi CDN (La Liga)",
        "matches": []
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for event_data in data.get("events", []):
                ...
                # FİLTRE: İçinde "la liga", "laliga" veya "primera" geçenleri yakala, alt ligleri ("segunda") çöpe at
                path_names = [str(p.get("name", "")).lower() for p in event.get("path", [])]
                is_laliga = any(("la liga" in p or "laliga" in p or "primera" in p) and "segunda" not in p for p in path_names)
    ...
    with open("matches.json", "w", encoding="utf-8") as f:
        ...
        print(f"İşlem Tamam! {len(final_data['matches'])} La Liga maçı başarıyla çekildi.")
